    ...
    return(
        <button className={`w-[20vw] h-[20vw] sm:w-[15vw] sm:h-[15vw] md:w-[8vw] md:h-[8vw] focus:outline-green-400 flex justify-center items-center p-2 border-gray-400${borderStyles ? ' ' + borderStyles : ''}`} onClick={onAddStep}>
            ...
            {fieldValue === fieldValues.x &&
                <svg xmlns="http://www.w3.org/2000/svg" className={`w-full h-full ${winner ? (winner.value === fieldValues.x && winner.winningCombination.includes(index)) ? 'text-green-600' : 'text-gray-300' : 'text-red-600'}`} viewBox="0 0 24 24" strokeWidth="0.9" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
            ...
            {fieldValue === fieldValues.o &&
                <svg xmlns="http://www.w3.org/2000/svg" className={`w-full h-full ${winner ? (winner.value === fieldValues.o && winner.winningCombination.includes(index)) ? 'text-green-600' : 'text-gray-300' : 'text-blue-600'}`} viewBox="0 0 24 24" strokeWidth="1" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
